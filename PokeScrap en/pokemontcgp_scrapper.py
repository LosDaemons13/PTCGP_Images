...
sets_info = {
	...
	"B3": {"name": "Pulsing Aura", "max_cards": 234}
}
...
def set_initial_card_index(set_id):
	...
	if set_id == "A1a":
		...
		# Starts after B2b (ends at 2945) with a +6 gap
		cardIndex = 2951
	else:
		cardIndex = 0
...
def extract_id(soup):
	...
	# Fins de sets et sauts suivants
		...
		return currentCardIndex + 6
	elif currentCardIndex == 2687:  # end B2
		cardIndex = 2693
		return 2687
		...
		return currentCardIndex + 6
	elif currentCardIndex == 2945:  # end B2b
		cardIndex = 2951
		return 2945
...
def extract_wp_gp_eligible(id_set, set_details, rarity):
	...
	isShiny = (
		...
		"(B2b)" in set_details and int(id_set) >= 87 or
		"(B3)" in set_details and int(id_set) >= 205
	)
